...
export const TeamPresencesPanel: React.FC<TeamPresencesPanelProps> = ({ tournamentId, teamId }) => {
  ...
  if (loading) return <p className="text-sm text-slate-500">Cargando partidos del equipo…</p>;
  ...
  return (
    <div className="space-y-3">
      <div>
        <h3 className="text-base font-semibold text-[#0F2A33]">Presencias de tu equipo</h3>
        <p className="text-xs text-slate-500">
          Marcá quiénes jugaron cada partido (antes o después de jugarlo). Solo el dueño del equipo puede cargarlas.
        </p>
      </div>
      {matches.length === 0 ? (
        <p className="text-sm text-slate-500">Tu equipo todavía no tiene partidos asignados en este torneo.</p>
      ) : (
            ...
            return (
              ...
              >
                ...
                <button
                  ...
                  className="shrink-0 rounded-lg bg-[#2E7D32] px-3 py-1.5 text-xs font-medium text-white hover:bg-[#256628]"
                >
                  Presencias
                </button>
              </li>
