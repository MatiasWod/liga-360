...
export const StandingsTable: React.FC<StandingsTableProps> = ({ rows, theme = 'dark', className = '' }) => {
  ...
  const textClassName = isDark ? 'text-text-primary' : 'text-slate-700';
  const headerClassName = isDark ? 'text-text-muted' : 'text-slate-500';
  const rowClassName = isDark ? 'border-border-subtle' : 'border-slate-100';
  const highlightClassName = isDark ? 'bg-accent-soft' : 'bg-brand-green/10';
        ...
        <tbody>
          {rows.map((row) => (
            <tr
              ...
              className={`border-b last:border-b-0 ${rowClassName} ${row.position <= 2 ? highlightClassName : ''}`}
            >
